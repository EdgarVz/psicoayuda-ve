...
import type { NestedPsychologistProfile, NestedPsychologistRequestProfile } from '@/types/database'
...
export async function getPatientRequests(userId: string): Promise<PatientRequestView[]> {
  ...
  return (data ?? []).map((row) => {
    const psy = row.psychologist_profiles as unknown as NestedPsychologistProfile & { profiles: NestedPsychologistRequestProfile }
    return {
      id: row.id,
...
export async function getPsychologistRequests(userId: string): Promise<PsychologistRequestView[]> {
  ...
  return (data ?? []).map((row) => ({
    id: row.id,
    patientName: (row.profiles as unknown as NestedPsychologistRequestProfile).display_name,
